...
public class ClientDao {
    ...
    private static final String FIND_CLIENTS_BY_VEHICLE_QUERY = "SELECT DISTINCT Client.id, Client.nom, Client.prenom, Client.email, Client.naissance FROM Client INNER JOIN Reservation ON Reservation.client_id = Client.id WHERE vehicle_id=?;";
    ...
    public ArrayList<Client> findClientsByVehicle(long id_vehicle) throws DaoException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Client client = new Client(client_id, nom, prenom, email, naissance);
                listClient.add(client);
            }
            return listClient;
        } catch (SQLException e) {
